...
| **DependencyTree**                   | FULLY_FUNCTIONAL | Hierarchical activity visualization. Priority filtering (Running > Failed > Pending > Completed), depth-aware prefixes, root-level priority sorting, partial phase collapse under viewport pressure (fan-out phases with 4+ children summarize when viewport is full)                                                                                                                                                                                                                                                                                                                                                                         |
...
| **teatest E2E**         | FULLY_FUNCTIONAL | `tui/teatest_helpers_test.go` — drives the real Bubble Tea program loop via `teatest/v2` (10 tests: startup, scroll, help toggle, quit, ctrl+c, WindowSizeMsg, VT screen assertion, deadlock regression, goroutine cleanup)                                                                                                                                      |
...
| **Deduplication**     | FULLY_FUNCTIONAL | Zero clones at standard `t=4`; strict type-aware `t=1` audit has 20 accepted minimum-idiom groups    |
