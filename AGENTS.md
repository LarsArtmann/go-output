...
- **InlineRenderer locking model (two mutexes, on purpose)**: `tickMu sync.RWMutex` guards config + lifecycle fields; `renderMu sync.Mutex` serializes terminal writes + `prevLines`. They MUST stay separate — merging them reintroduces the `Stop()` deadlock (the refresh loop's `Draw()` takes `tickMu.RLock`, so holding the write lock across `<-tickerDone` would deadlock). All config reads go through one `snapshotConfig()` call returning an immutable `rendererConfig` (one `RLock`, not scattered field reads). Setters (`SetStartTime`/`SetAppName`/`SetNoColor`/`SetHideCursor`/`SetMaxHeight`) take `tickMu.Lock` and are safe to call during the render loop.
