...
- **Semantic color delegation**: `tui/colors.go` delegates its 4 activity-state colors (`success`/`warning`/`err`/`dim`) to `nom.Colors` (`Completed`/`Running`/`Failed`/`Paused`) — nom is the single source of truth for the palette. TUI-only colors (title, selection, help) stay local. Do not re-duplicate ANSI codes between nom and tui.
