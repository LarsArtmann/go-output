...
- **Sealed Event sum type (v0.17.0)**: `nom.Event` carries an unexported `isEvent()` marker — only the 7 concrete structs in `nom/event.go` can implement it. Handlers route via exhaustive Go type switch, not string dispatch. Adding a new event type means the compiler errors on every non-exhaustive switch — this is by design. Never re-introduce `GetEventType()` string dispatch or accessor interfaces.
