...
func TestTable(t *testing.T) {
	t.Parallel()
	runSubtest(t, "RowCount and ColCount", testTableRowColCount)
	runSubtest(t, "CreateRowEdges", testTableCreateRowEdges)
	runSubtest(t, "ToMapSlice", testTableToMapSlice)
	runSubtest(t, "Footer", testTableFooter)
}

func testTableRowColCount(t *testing.T) {
	t.Helper()

	data := NewTable([]string{"Name", "Value", "Count"})
...
func testTableCreateRowEdges(t *testing.T) {
	t.Helper()
...
func testAssertNilRowEdges(t *testing.T, data *Table, desc string) {
...
func testCreateRowEdgesMultiple(t *testing.T) {
	...
	verifyEdge := func(idx int, from, to string) {
		if edges[idx].From.Get() != from || edges[idx].To.Get() != to {
			t.Errorf(
				"Edge %d = {%s, %s}, want {%s, %s}",
				idx,
				edges[idx].From.Get(),
...
func testAssertToMapSliceNil(t *testing.T, data *Table, desc string) {
	t.Helper()
...
func testTableToMapSlice(t *testing.T) {
	t.Helper()
...
func testTableFooter(t *testing.T) {
	...
	t.Run("set and get footer", func(t *testing.T) {
		...
		data := NewTable([]string{"Name", "Count"})
...
func TestTableNilMethods(t *testing.T) {
	t.Parallel()
...
func TestTableValidate(t *testing.T) {
	...
	buildData := func(headers, footer []string) *Table {
		data := NewTable(headers)
		if footer != nil {
	...
	for _, tt := range []struct {
		...
		{name: "empty headers with footer", data: &Table{Footer: []string{"x"}}, wantErr: false},
		...
		{name: "nil row", data: &Table{Headers: []string{"A"}, Rows: [][]string{nil}}, wantErr: true},
		{name: "nil row after valid row", data: &Table{Headers: []string{"A", "B"}, Rows: [][]string{{"1", "2"}, nil}}, wantErr: true},
	} {
