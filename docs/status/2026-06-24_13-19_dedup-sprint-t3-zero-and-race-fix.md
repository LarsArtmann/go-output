...
The first version of `transitionTask` returned the activity so the caller could call `SetCompleted()`/`SetFailed()` *outside* the write lock:
...
**Fix**: pass a closure that runs *under* the write lock:
...
`nix run .#test-race` now passes for `nom` and `tui`. Worth investigating whether this race was *introduced* by my refactor or was *exposed* by it — the original code (`Lock; defer; getOrCreate; applyCountsDelta; SetCompleted` all in sequence under the same `defer ns.mu.Unlock()`) was race-free. My first attempt moved `SetCompleted` outside the lock, which broke the invariant. The closure version preserves the original behavior exactly.
