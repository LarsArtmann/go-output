...
- **(B)** Return the raw pointer and document "caller must not mutate" (current behavior, but the TUI _does_ mutate it — e.g., `model.nomSubscriber.GetDependencyTree().AddActivity(...)`)?
