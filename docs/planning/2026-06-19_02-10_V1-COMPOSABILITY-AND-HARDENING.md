...
1. **The user's #1 complaint:** _"we have TOO MANY files in root already and this SDK is not
   composable enough."_ Root is a god-package: **16 production files, 1908 lines**, mixing core
...
**The decisive constraint:** root extraction is a _breaking_ change. If we freeze v1.0.0
...
1. **Core Invariant stays sacred:** root never imports a sub-module. Extraction _moves_ markdown/tree OUT; it does not make root import them back. After extraction, root's `render_tabledata.go` init() for markdown/tree is **removed** — the new modules self-register like `table/`.
