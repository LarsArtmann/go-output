...
- **State** — `NOMStyleSubscriber` owns the activity map (`map[ActivityID]*Activity`), the `DependencyTree`, and the `TimingCache` under one `sync.RWMutex`. `Activity` is the single source of truth — it embeds `output.GraphNode` (for diagram export) plus temporal domain fields (Status, StartTime, EndTime, Err, etc.). `ActivityNode` in the tree embeds `*Activity` (shared pointer), so mutations via `SetRunning()`/`SetCompleted()` are instantly visible to both the subscriber and the tree with zero sync overhead.
