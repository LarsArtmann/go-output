...
| Ordering            | Insertion order                     | **Self-resorting**: Failed > Running > Pending > Completed; ties by elapsed-then-ID (`tree_priority.go`)                                                                                                                                                       |
| Time-awareness      | None                                | Persistent cross-run CSV at `~/.cache/nom-timing.csv`; **median** of last ≤10 runs predicts pending durations                                                                                                                                                  |
| State model         | Single (`TreeNode`)                 | `ActivityNode` holds only an `ActivityID` (no shared pointer). All mutable fields consumed via immutable `ActivitySnapshot` copies taken under subscriber read lock (`SnapshotActivities()`). Eliminates the data race of the former shared-`*Activity` model. |
...
- **Ingestion** — `nom.Event` is a sealed sum type with unexported `isEvent()` marker. Handlers route via exhaustive Go type switch on concrete event structs (`WorkflowStarted`, `ActivityStarted`, `ActivityCompleted`, `ActivityFailed`, etc.). No string dispatch, no accessor interfaces. Adding a new event type produces compile errors on every non-exhaustive switch.
- **State** — `NOMStyleSubscriber` owns the activity map (`map[ActivityID]*Activity`), the `DependencyTree`, and the `TimingCache` under one `sync.RWMutex`. `Activity` holds temporal domain fields (Status, StartTime, EndTime, Err, etc.). Elapsed time is derived at snapshot time via `activity.elapsedAt(now)` — no per-tick mutation. `ActivityNode` in the tree holds only an `ActivityID` (no shared pointer); all mutable fields are consumed via immutable `ActivitySnapshot` copies taken under the subscriber read lock (`SnapshotActivities()`). Activity counts are incrementally cached (O(1)) via `applyCountsDelta` on every state transition.
