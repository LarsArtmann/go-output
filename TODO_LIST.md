...
| #   | Task                                                                    | Resolution                                                                                                                                                                                                                                                                                                                                 |
| --- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | **Fix TUI test deadlock** — teatest E2E test hangs in CI                | **Fixed.** Two root causes: (1) `waitForVisible` (used by 5 of 9 tests) still called `teatest.WaitFor` → `io.ReadAll`, which blocks indefinitely when the tick loop writes continuously (buffer never empties for EOF). Replaced with `pollTeatestOutput` (bounded single-Read polling). (2) Cleanup called only `tm.Quit()` without `tm.WaitFinished()`, leaking the program goroutine. Added `WaitFinished(3s)` to cleanup. All 10 teatest tests pass under `-race` in 2s (was 600s hang). |
