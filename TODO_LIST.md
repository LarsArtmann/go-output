...
| #   | Question                                                                                                                                                                                               |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 15  | **Should `TableData` use exported fields or getters for v1?** Current: both exist (`Headers` + `GetHeaders()`). Option A: exported fields only. Option B: unexported + validated setters (impossible-state-proof). Option C: keep both for v0.x. Affects every consumer; v1 stability commitment. *(See also: improve-codebase-architecture deepening report — unexporting fields makes column-mismatch unrepresentable.)* |
