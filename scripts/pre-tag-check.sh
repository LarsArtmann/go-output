...
if [ -n "$LATEST_ROOT" ]; then
	...
	if [ "$ACTUAL" -ne "$EXPECTED" ]; then
		fail "latest release $LATEST_ROOT has $ACTUAL tags, expected $EXPECTED (root + ${#TAGGED_SUBMODULES[@]} submodules). Missing: $(comm -23 <(
			echo "$LATEST_ROOT"
			printf '%s/v%s\n' "${TAGGED_SUBMODULES[@]}" "$LATEST_VER" | sort
		) <(git tag -l "*v${LATEST_VER}" | sort))"
	fi
