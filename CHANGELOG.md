...
- **ci** — Bump all GitHub Actions to Node.js 24-compatible majors (checkout v4→v7, setup-go v5→v7, golangci-lint-action v7→v9, action-gh-release v1→v3) to eliminate Node.js 20 deprecation warnings.
- **ci** — Add `-timeout 120s` to all test steps (ci.yml + release.yml) to fail fast on deadlocks instead of hitting the 10-minute default.
- **ci** — Lower art-dupl threshold from t=50 to t=4 (the production dedup gate per ADR 008) for meaningful duplication detection.
