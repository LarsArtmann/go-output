...
**Rationale:** This is a Go library with 8 independent modules. Each module has its own `go.mod` with `replace` directives. Running Go checks in Nix would require either `gomod2nix` (8 configs) or `buildGoModule` (8 vendorHashes). The complexity is not worth it — CI already handles these checks reliably.
...
| 6 | Update `README.md` with nix commands | ✅ Done |
| 7 | Update `AGENTS.md` (nix commands, drop justfile refs) | ✅ Done |
| 8 | Handle `.pre-commit-config.yaml` conflict | ✅ Gitignored — auto-generated by git-hooks.nix |
| 9 | Delete `justfile` | ✅ Deleted |
| 10 | Update CI to use nix (optional) | ⬜ Skipped — CI uses golangci-lint-action + govulncheck |

---

## Module Inventory (8 modules, unchanged)
